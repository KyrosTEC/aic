...
class FirstPolicy(Policy):
    ...
    def _pose_with_rotation_offset(
        self,
        base: Pose,
        roll: float = 0.0,
        pitch: float = 0.0,
        ...
        """Return a copy of base pose with a small RPY rotation offset applied.

        Angles are in radians. Start with small values:
          0.05 rad = 2.9 degrees
          0.10 rad = 5.7 degrees
          0.15 rad = 8.6 degrees

        ROS geometry_msgs/Quaternion stores orientation as x, y, z, w.
        SciPy also returns quaternions as x, y, z, w.
        """
        p = self._copy_pose(base)
        ...
        # Apply the small rotation offset in the base/world convention.
        # If the direction feels reversed in Gazebo, change this to:
        #     r_new = r_current * r_delta
        r_new = r_delta * r_current
        q_new = r_new.as_quat()

        p.orientation.x = float(q_new[0])
        p.orientation.y = float(q_new[1])
        p.orientation.z = float(q_new[2])
        p.orientation.w = float(q_new[3])
        return p
    ...
    def insert_cable(
        ...
        # Optional orientation search. These are small pitch offsets in radians.
        # Use pitch first for "tip up/down" behavior. If it rotates in the wrong
        # physical direction, swap signs or try roll instead.
        rotation_offsets = [
            (0.0, 0.0, 0.0),
            (0.0, 0.05, 0.0),
            (0.0, -0.05, 0.0),
        ...
        found_depth = False
        for ridx, (roll_off, pitch_off, yaw_off) in enumerate(rotation_offsets):
            rotated_search_base_pose = self._pose_with_rotation_offset(
                search_base_pose,
                roll=roll_off,
                pitch=pitch_off,
                yaw=yaw_off,
            ...
            send_feedback(
                f"Rotation search {ridx}: roll={roll_off:.3f}, pitch={pitch_off:.3f}, yaw={yaw_off:.3f}"
            )

            # Move to the rotated orientation before trying insertion.
            self._move_and_wait(
                move_robot,
                rotated_search_base_pose,
                wait_s=0.25,
                stiffness=self.free_space_stiffness,
                damping=self.free_space_damping,
            )
            ...
            for idx, (dx, dy) in enumerate(search_offsets):
                ...
                if force > self.force_stop_n:
                    ...
                    lift = self._pose_with_offset_negative(rotated_search_base_pose, dx, dy, 0.008)
                    self._move_and_wait(
                        move_robot,
                        lift,
                        wait_s=0.25,
                ...
                # Move horizontally above the search offset with the current rotation.
                above = self._pose_with_offset_negative(rotated_search_base_pose, dx, dy, 0.0)
                self._move_and_wait(
                    move_robot,
                    above,
                    wait_s=0.15,
                ...
                # Main insertion attempt: move downward incrementally with lower stiffness.
                reached_depth = self._gentle_insert_attempt(
                    get_observation=get_observation,
                    move_robot=move_robot,
                    send_feedback=send_feedback,
                    base_pose=rotated_search_base_pose,
                    dx=dx,
                    dy=dy,
                    max_depth=max_depth,
                    step=step,
                ...
                # Keep track of the best final pose achieved during this search iteration.
                best_final_pose = self._pose_with_offset_negative(
                    rotated_search_base_pose, dx, dy, -max_depth
                )
                if reached_depth:
                    send_feedback(
                        f"Reached full insertion depth at rotation {ridx}, search {idx}; holding pose"
                    )
                    found_depth = True
                    break
